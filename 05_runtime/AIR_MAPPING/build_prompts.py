...
## 3. 태스크를 보기 전에 한 번 부른다 — HARD RULE
...
{note_block}
## 8. 도구를 다 부른 다음, JSON 하나를 낸다 — HARD RULE
...
def build(key, raw=False):
    ...
    sk = SKELETON
    if raw:
        i = re.search(r'^## \d+\. 도구가 돌려주는 것', sk, re.M).start()
