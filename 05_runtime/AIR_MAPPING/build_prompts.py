...
- `source_footer` 가 `""` 인 레코드도 **정상이고 버리지 않는다.**
  07 이 그것을 「{source} KB 검색결과」로 싣는다. 파일명 없이도 근거가 된다.
