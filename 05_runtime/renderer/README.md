...
결과 JSON을 **이어서 붙여넣거나** 파일 여러 개를 끌어다 놓으면,
표지와 이슈별 섹션으로 나뉜 문서 하나가 만들어진다. 이슈마다 새 페이지에서 시작한다.
문서 제목은 선택 입력란에 적으면 표지에 들어간다.

결과 JSON이 아닌 블록이 섞여 있으면 건너뛰고 몇 개를 건너뛰었는지 알려준다.
...
이슈가 2건 이상이면 앞에 표지가 붙는다.

```
[표지]  문서 제목 + □ 대상 이슈 목록
[새 페이지]  <#6 …>  이슈 1 본문
[새 페이지]  <#3 …>  이슈 2 본문
[새 페이지]  [부록] 추적정보 — 이슈별 표 (--trace 일 때만)
```
...
기본 파일명은 이슈 ID를 이어 붙인다: `MRI_S06.docx`, `MRI_S06_S03.docx`.
5건 이상이면 `MRI_5issues.docx`.

**기본 파일명에 한글을 쓰지 않는다.** 비ASCII 파일명을 통째로 버리면서 확장자까지
없애버리는 브라우저가 있다(확인: Chromium 에서 `MRI_대응안_2건.docx` → `download`).
직접 지정할 때는 `-o` 로 한글 이름을 써도 된다.
...
- 브라우저판: Chromium으로 실제 페이지를 띄워 변환시킨 `.docx` 가
  ZIP 무결성·OOXML 스키마 검증 통과. JS 오류 없음. 400px 폭에서 가로 스크롤 없음.
  단건·2건 합본·추적정보 포함·비정상 블록 혼입을 각각 확인했다.
  - 검증 과정에서 `w:pPr` 자식 요소 순서 위반을 발견해 수정했다.
    순서는 `pageBreakBefore -> pBdr -> spacing -> ind` 로 스키마가 강제한다.
  - 한글 기본 파일명이 버려지는 문제를 발견해 ASCII 파일명으로 바꿨다.
- 명령줄판 합본: 단건 33문단 / 2건 70문단 / 2건+추적정보 77문단, 모두 스키마 검증 통과.
